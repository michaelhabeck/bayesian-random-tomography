...
from .likelihood import Normal, LowerUpper, Logistic, Relu, Exponential
...
class ChromosomeSimulation(object):
    ...
    def create_radius_of_gyration(self, Rg=0., model='normal'):
        radius = RadiusOfGyration()
        if model == 'normal':
            return Normal(radius.name, np.array([Rg]), radius,
                          params=self.params)
        elif model == 'exponential':
            return Exponential(radius.name, np.array([Rg]), radius,
                               params=self.params)

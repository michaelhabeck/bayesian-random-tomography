...
"""
from ._isd import nblist
from .core import ctypeproperty, CWrapper

class NBList(CWrapper):
    """NBList

    Neighbor list that allows the efficient evaluation of pairwise through-space
    interactions which have a finite range. Particles are sorted into cubic
    cells whose size is equal or larger than the truncation threshold of the
    nonbonded interaction. After assigning particles to cells, the evaluation
    of the pairwise interactions can be restricted to the particles that reside
    in neighboring cells. By this trick the evaluation of pairwise interactions
    no longer has a complexity that is quadratic in the number of particles, but
    ...
    def __init__(self, cellsize, n_cells, n_per_cell, n_particles):
        ...
        Parameters
        ----------
        cellsize : positive float
            Length of a single cell. 

        n_cells : positive int
            Number of cells in one dimension. 
            
        n_per_cell : positive int
            Maxixmum number of atoms assignable to one cell. 
    ...
    @ctypeproperty(int)
    def n_cells():
        """Number of cubic cells in each spatial direction such that the total
        number of cells is 'n_cells^3'.
        """
        pass

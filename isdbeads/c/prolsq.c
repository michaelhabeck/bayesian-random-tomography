...
static double
prolsq_energy(PyForceFieldObject *self, double d, double d0, double k) {
...
static double
prolsq_gradient(PyForceFieldObject *_self, double d, double d0, double k,
		double *E) {
...
static double
energy(PyProlsqObject *self, 
       double *coords, 
       int *types, 
       int n_particles) {
  
  return forcefield_energy((PyForceFieldObject*)self, coords, types,
			   n_particles);
}

static int
gradient(PyProlsqObject *self, 
	 double *coords, 
	 double *forces, 
	 int *types, 
	 int n_particles, 
  ...
  return forcefield_gradient((PyForceFieldObject*)self, coords, forces, types,
			     n_particles, E_ptr);
}

static PyObject*
py_energy(PyProlsqObject *self, PyObject *args) {
  
  PyArrayObject *coords, *types;

  if (!PyArg_ParseTuple(args, "O!O!", &PyArray_Type, &coords, &PyArray_Type,
			&types)) {
    RAISE(PyExc_ValueError,
	  "numpy arrays storing coordinates and atom types expected.",
	  NULL);
  }
...
static PyObject*
py_update_gradient(PyProlsqObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!O!O!i", 
    ...
    RAISE(PyExc_TypeError,
	  "numpy arrays storing coordinates, forces and atom types expected.",
	  NULL);
  }
  if (calculate_energy) {
    gradient(self, 

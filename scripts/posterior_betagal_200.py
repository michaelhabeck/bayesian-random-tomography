...
margin = 20
images = reader.read().data.copy()[margin:-margin, margin:-margin]
pixelsize = 3.54
...
pklfile = '../simulations/posterior_betagal_100.pkl'
if os.path.exists(pklfile):
    print('loading {}'.format(pklfile))
    ensemble = load(pklfile)
    set_params(posterior, ensemble)
    coords = params['coordinates'].get().reshape(n_particles, 3)
